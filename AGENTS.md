...
> **System Admin transaction access:** System Admin is granted read-only access to transactions (list and detail) so the role is not blocked when browsing and can support troubleshooting. It can never create, edit, submit, delete, approve, reject, or void transactions — enforced in the backend service layer.
